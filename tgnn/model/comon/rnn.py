...
# Author: chenchenqin
# Data: 2022/10/24 11:47
import torch
import torch.nn as nn


def truncated_normal(size, num_resample=5):
    x = torch.empty(size + (num_resample,)).normal_()
...
class RNN(nn.Module):
    def __init__(
        ...
        self.rnn = nn.LSTM(*args, bidirectional=bidirectional, bias=bias, **kwargs)
        self.init_weights(disable_state_bias=disable_state_bias)
    ...
    def init_biases(self, types=('bias_ih',)):
        for name, param in self.rnn.named_parameters():
            if any(k in name for k in types):
                with torch.no_grad():
                    param.set_(0.5 * truncated_normal(param.shape).to(param))
        ...
        for name, x in self.rnn.named_parameters():
            if any(k in name for k in types):
                for i in range(0, x.size(0), self.rnn.hidden_size):
                    nn.init.orthogonal_(x[i:i + self.rnn.hidden_size])
